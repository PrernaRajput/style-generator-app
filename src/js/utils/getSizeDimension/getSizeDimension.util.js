...
export const getSizeDimension = ( args = {} ) => {
    ...
    const sizeDimensions = { //standard size dimensions for vh, vw
        xl: '100',
        l: '90',
        m: '70',
        s: '50'
    ...
    if ( DIMENSION_STANDARDS[ value ] ) { //process value by check against dimension standards
        
        return ( addStyle( dimensionType, DIMENSION_STANDARDS[ value ] ) );
        
    } else if ( ( DATA_TYPES.STRING === typeof value ) && ( some( LAYOUT_STYLES, key => value.includes( key ) )  ) ) { // Checks if the 'value' is a string and contains layout style keys.
        ...
        if ( !isEmpty( matchedKey ) ) {
            ...
            return addStyle( `${dimensionType}-${matchedKey}`, getObjectKey( updatedValue, sizeDimensions ) );

        }
    } else if ( null !== value && value !== undefined ) { // Checks if 'value' is neither null nor undefined
